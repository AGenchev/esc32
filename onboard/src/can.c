...
static int8_t canSend(uint32_t id, uint8_t tid, uint8_t seqId, uint8_t n,
		void *data)
{
	int8_t mailbox;
	uint32_t *d = data;
	...
	if (mailbox >= 0)
	{
		CAN_CAN->sTxMailBox[mailbox].TIR = id | (canData.networkId << 14)
				| ((tid & 0x1f) << 9) | (seqId << 3) | CAN_Id_Extended;
...
{
	canSend(CAN_LCC_NORMAL | CAN_TT_NODE | CAN_FID_NACK, pkt->sid, pkt->seq, 0,
			0);
}

static inline void canAck(canPacket_t *pkt)
{
	canSend(CAN_LCC_NORMAL | CAN_TT_NODE | CAN_FID_ACK, pkt->sid, pkt->seq, 0,
			0);
}

static inline void canReply(canPacket_t *pkt, uint8_t size)
{
	canSend(CAN_LCC_NORMAL | CAN_TT_NODE | CAN_FID_REPLY | (pkt->doc << 19),
			pkt->sid, pkt->seq, size, pkt->data);
}
...
static inline void canSendGetAddr(void)
{
	...
	*((uint32_t *) &d[0]) = canData.uuid;
	...
	canSend(CAN_LCC_NORMAL | CAN_TT_NODE | CAN_FID_REQ_ADDR, 0, canGetSeqId(),
			6, d);
}
...
static inline void canProcessGet(canPacket_t *pkt)
{
	...
	case CAN_DATA_PARAM_ID:
		paramId = *((uint16_t *) pkt->data);
		if (paramId < CONFIG_NUM_PARAMS)
		{	float val;
			switch(configFormatTypes[paramId]) // param type identifier
					{
					case 'u':
						val = parameters_asUint32[paramId];
						break;
					case 'i':
						val = parameters_asSint32[paramId];
						break;
					default:
						val = parameters_asFloat32[paramId];
					}
			*(float *) (pkt->data) = val;
			canReply(pkt, 4);
		}
