...
#define DEFAULT_PTERM			0.25f       // The P term for the RPM PI controller
#define DEFAULT_PNFAC			10.0f		// ???
#define DEFAULT_ITERM			0.0006f     // The I term for the RPM PI controller
...
// but don't seek support for non-original ESC32 instances !!!
...
#define DEFAULT_GOOD_DETECTS_START	75.0f	  // 	Once started, the number of good/in order zero crossings needed to be detected before the motor is considered to be in the running state.
#define DEFAULT_BAD_DETECTS_DISARM	48.0f	  // The number of missed zero crossing detects allowed before the ESC considers the motor not to be running at which point will go into the disarmed state.
...
#define DEFAULT_SWITCH_FREQ		20.0f	    // The output PWM pulse frequency used to power the motor windings in KHz.  Valid range is from 4KHz to 64KHz.
...
#define DEFAULT_PWM_MIN_PERIOD		2200	// PWM interface: The minimum period in microseconds that the ESC will consider an input PWM waveform to be valid.  Default value represents approx 450Hz.  For MultiWii and ArduCopter APM need to be 2000
#define DEFAULT_PWM_MAX_PERIOD		25000	// PWM interface: The maximum period in microseconds that the ESC will consider an input PWM waveform to be valid.  Default value represents approx 40Hz

#define DEFAULT_PWM_MIN_VALUE		750	    // PWM interface: The minimum input PWM pulse length in microseconds which the ESC will consider to be valid. For DJI Naza this value has to be 900!
#define DEFAULT_PWM_LO_VALUE		1000	  // PWM interface: The input PWM pulse length in microseconds for the lowest throttle setting. For ArduCopter APM (or Open Pilot Revo w/ APM fw) this value needs to be changed to 1035!
#define DEFAULT_PWM_HI_VALUE		1950	  // PWM interface: The input PWM pulse length in microseconds for the highest throttle setting.
#define DEFAULT_PWM_MAX_VALUE		2250	  // PWM interface: The maximum input PWM pulse length in microseconds which the ESC will consider to be valid.
#define DEFAULT_PWM_MIN_START		1100	  // PWM interface: The input PWM pulse length in microseconds at which the motor will be started.  Once running, the throttle can be brought as low as PWM_MIN_VALUE as long as the motor does not stall.

#define DEFAULT_PWM_LOWPASS		0.0f	    // PWM interface: lowpass on PWM input values (0 = none, 10 = heavy, no upper limit)
#define DEFAULT_RPM_MEAS_LP		0.5f	    // PWM interface: lowpass measured RPM values for closed loop control (0 = none, 0.99 = max, >=1 not allowed)

#define DEFAULT_PWM_RPM_SCALE		6500	  // PWM interface: The scale of the input PWM pulse length.  In closed loop RPM run mode, PWM_LO_VALUE will indicate 0 RPM and PWM_HI_VALUE will indicate this RPM. Closed loop modes only! RPM equivalent of maximum PWM IN in CLOSED_LOOP mode
...
enum configParameters {
    CONFIG_VERSION = 0,
    STARTUP_MODE, // 0 == normal open loop mode 1 == closed loop RPM mode 2 == thrust closed loop mode
    BAUD_RATE,    // The default baud rate
    PTERM,        // Speed PI control, P item
    ITERM,        // Speed PI control, item I
    ...
    MIN_PERIOD,       // The minimum commutation period
    MAX_PERIOD,		  // The maximum commutation period
    BLANKING_MICROS,  // BEMF sensing blanking time
    ...
    START_VOLTAGE,
    GOOD_DETECTS_START, // how many good detects're necessary to consider motor running OK ?
    BAD_DETECTS_DISARM, // how many bad detects are enough to stop the motor from running
    MAX_CURRENT,
    SWITCH_FREQ,        // PWM freq in KHz, default 20KHz
    MOTOR_POLES,        // N of magnetic poles, used to detect RPM and ...
    PWM_MIN_PERIOD,
    PWM_MAX_PERIOD,
    PWM_MIN_VALUE,      // PWM input minimum high time (pulse width)
    PWM_LO_VALUE,
    PWM_HI_VALUE,
    PWM_MAX_VALUE,     //  maximum high time (pulse width)
    PWM_MIN_START,
    PWM_RPM_SCALE,
    FET_BRAKING,       // Brake Mode 0 = Off 1 = On
    PNFAC,
    INFAC,
    THR1TERM,
    THR2TERM,
