...
{    // parse & execute
	switch ( (binaryCommands_t) commandBuf.command )
	{
	...
	case BINARY_COMMAND_ARM:
		...
		{
			inputMode = ESC_INPUT_UART;       // switch to serial control mode
			runArm();
			binaryAck();
		}
		else
	...
	case BINARY_COMMAND_START:
		if (ESC_state == ESC_STATE_DISARMED || ESC_state > ESC_STATE_STOPPED)
		{
			binaryNack();
		}
		else
	...
	{
		uint32_t freq = (uint32_t) commandBuf.params[0];
	...
	case BINARY_COMMAND_TELEM_VALUE:
		if (commandBuf.params[0] < BINARY_VALUE_NUM	&& commandBuf.params[1] < BINARY_VALUE_NUM)
		{
			...
			binaryTelemValues[ (unsigned int)commandBuf.params[0] ] = commandBuf.params[1];
	...
	case BINARY_COMMAND_SET:  // // set the corresponding parameters
		if (ESC_state <= ESC_STATE_STOPPED 	&& configSetParamByID((unsigned int) commandBuf.params[0],	commandBuf.params[1]))
		{
			binaryAck();
		}
		else
...
void binaryCommandRead(void)
{
	...
	{   // the serial port has received data
...
{  // called inside Main().  Serial data exchange binary operation mode
	static uint32_t telemRowsCount;
		...
		for (i = 0; i < BINARY_VALUE_NUM; i++)
		{
			if (binaryTelemValues[i] != BINARY_VALUE_NONE)
			{
				...
				case BINARY_VALUE_IDLE_PERCENT:
					binarySendFloat(U_idle_x100/100);
					break;

				case BINARY_VALUE_STATE:
					binarySendFloat((float) ESC_state);
					break;
