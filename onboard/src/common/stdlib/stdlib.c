...
/****************************************************************/
uint32_t strtoul_r(char *str, char **ptr, int base)
{
...
void * memcpy(void *dest, const void *src, unsigned n)
{
	...
	if ((dest != NULL) && (src != NULL) && (n > 0))
	{
		bytes = (n & 0x3);        // remainder
		longs = (n - bytes) >> 2; // (bytes-remainder) div 4

		while (longs--)			  // DWORD copy
			*dpl++ = *spl++;
