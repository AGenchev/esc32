...
/* Translated from Chinese:
  * This file is 1-wire communication protocol implementation, in pwm.c, will produce pwm capture interrupt, and then call owEdgeDetect function. To send and receive data, processing
  * Reset, initialize:
  * 1, owReset must first call this function
  * 2, inputMode set the variable, the input mode is ow
  * 3, owResetIsr for further initialization
  *
  * Receiving data:
  * 1, pwm.c file, PWM_IRQ_HANDLER interrupt function, will call owEdgeDetect function
  * 2, owEdgeDetect to determine whether to receive data or send data, if it is to receive data
  * 3, owReadBitIsr function, read the value of GPIO, and then put the data into the owByteValue variable, while owBit plus 1,
  * 4, owByteReceived function, when owBit received after the completion of a byte, will call, and then continue to determine whether there is data to receive, if all received after the analysis of data
  * 5, command analysis
  * Data sent:
  * 1, pwm.c file, PWM_IRQ_HANDLER interrupt function, will call owEdgeDetect function
  * 2, owEdgeDetect to determine whether to receive data or send data
  */
#include "ow.h"
#include "main.h"
#include "timer.h"
...
uint8_t owROMCode[8];
uint8_t owBuf[32];        // The received data is actually put here.
uint8_t *owBufPointer;
uint8_t owState;       // Values: OW_RESET_STATE_0, OW_RESET_STATE_1, OW_RESET_STATE_2, OW_READ, OW_WRITE several different states generally only appear READ or WRITE situation
uint8_t owLastCommand;
uint8_t owByteValue;
uint8_t owReadNum;   // the number of data currently being read
uint8_t owWriteNum;  // the number of data currently being written
uint8_t owBit;
...
void owReadComplete(void)
{
	...
	case OW_PARAM_READ:
		...
		else
		{
			...
			{
				// copy config value into send buffer
				pointer = (uint8_t *) &(parameters[owBuf[1]]);
				owBuf[2] = pointer[0];
				owBuf[3] = pointer[1];
				owBuf[4] = pointer[2];
				owBuf[5] = pointer[3];
...
{
	// sample bus Read the GPIO and put the data in the owByteValue variable
